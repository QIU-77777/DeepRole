...
`src/maps/graybox.json` 使用与 Tiled 对象层相近的结构，暂时作为四张地图的占位数据。`src/maps/loader.ts` 同时接受当前灰盒集合和单张 Tiled JSON 导出物；正式地图接入时保持 `map_id`、出口 `id`、waypoint 和交互对象 ID 稳定，再替换数据文件。
