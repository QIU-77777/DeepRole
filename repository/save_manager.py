...
async def import_save_archive(save_filename: str) -> bool:
    ...
    try:
        ...
        # 向量库重建（EpisodeMemory + Understanding）属 memory 层职责，由调用方在文件恢复后触发。
        # 留在此处会让 storage 反向 import memory，形成 storage⇄memory 环（靠懒加载掩盖）。
        print(f"[读档] 文件已恢复（向量库重建交由调用方）: {save_filename}", flush=True)
        return True
...
async def export_save_archive_with_detail() -> tuple[str | None, str | None]:
    ...
    try:
        ...
        with zipfile.ZipFile(str(temp_path), "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            for marker in [".story_id", ".turn_counter.json", PLAYER_NAME_FILENAME, "spatial_state.json"]:
